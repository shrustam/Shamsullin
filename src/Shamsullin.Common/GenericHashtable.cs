...
{
    [Serializable]
    public class GenericHashtable<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary
    {
        ...
        /// </summary>
        public ICollection<TKey> Keys => _items.Keys.Cast<TKey>().ToList();
        ...
        /// </summary>
        public ICollection<TValue> Values => _items.Values.Cast<TValue>().ToList();
        ...
        object IDictionary.this[object key]
        {
            ...
            set { _items[(TKey)key] = value; }
        }

        public override string ToString()
        {
            var itemsStr = Keys.Select(x => $"{x}: {Items[x]}");
            var result = string.Join(Environment.NewLine, itemsStr);
            return result;
        }

        public T Clone<T>() where T : GenericHashtable<TKey, TValue>, new()
        {
            var result = new T();
            Items.Keys.Cast<TKey>().ForEach(x => result[x] = (TValue)Items[x]);
            return result;
        }
    }
}