...
namespace Shamsullin.Common.Extensions
{
    ...
    {
        public static string Dump(this object obj, int depth = 4,
            ...
            else
            {
                ...
                if (enumerableElement != null)
                {
                    foreach (var item in enumerableElement)
                    {
                        ...
                        {
                            _level++;
                            DumpElement(item, indentSize, depth,
                    ...
                    foreach (var memberInfo in members)
                    {
                        ...
                        else
                        {
                            ...
                            if (_level <= depth)
                            {
                                ...
                                if (!alreadyTouched)
                                    DumpElement(value, indentSize, depth,
                                        BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
                                else
                                    Write("{{{0}}} <-- bidirectional reference found", indentSize,
