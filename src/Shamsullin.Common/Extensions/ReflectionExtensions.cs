...
namespace Shamsullin.Common.Extensions
{
	public static class ReflectionExtensions
	{
		...
		public static object GetPropertyValue<T>(this T obj, string propertyName)
		{
			...
			foreach (var name in fullPath)
			{
				...
				if (name.Contains('[') && name.Contains(']'))
				{
					...
					var array = result.GetType().GetPropertiesEx().FirstOrDefault(x => x.Name == arrayName).With(x => x.GetValue(result, null)) as object[];
					if (array == null) return null;
					result = array[index];
				}
				else
				{
					result = result.GetType().GetPropertiesEx().FirstOrDefault(x => x.Name == name).With(x => x.GetValue(result, null));
				}
			}
		...
		public static TDest Map<TDest>(this Dictionary<string, object> source, Action<TDest> action = null) where TDest : class, new()
		{
			if (source == null) return null;
		...
		public static DataTable ToDataTable<T>(this IEnumerable<T> items, bool useSqlDateTime) where T : new()
		{
			...
			foreach (var item in items)
			{
				...
				foreach (var property in typeof(T).GetPropertiesEx())
				{
					...
					{
						row[property.Name] = ((DateTime)value) < SqlDateTime.MinValue.Value ? SqlDateTime.MinValue.Value : value;
					}
					else
					{
